...
let _ =
  let checks =
    [
      ...
      check_ocamlfind_package conf ~fail: false ~min_version: [2;4;1] "js_of_ocaml" ;
      conf.camlp4 <> "" ;
    ]
  in
  if List.fold_left (&&) true checks then
    (
     add_subst "PACKAGE_JS" "xmldiff.js";
     add_subst "LIB_JS" "xmldiff-js.cma";
     print_endline "xmldiff.js package will be compiled and installed"
    )
  else
    print_endline "xmldiff.js package will NOT be available"
